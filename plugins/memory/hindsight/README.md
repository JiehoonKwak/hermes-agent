...
| `retain_payload_mode` | `legacy` | `legacy` preserves the upstream role transcript. `provenance_v2` normalizes only Hermes-authored transport/runtime envelopes, restores exact mid-turn user steers, separates quoted context, and keeps direct-user vs agent-final authorship explicit. The main agent prompt is unchanged. |
...
Hermes selects and lazy-installs `hindsight-client == 0.8.6`. The setup manifest accepts `hindsight-client >= 0.8.6,<0.10`; session start attempts to auto-upgrade an older installation.
